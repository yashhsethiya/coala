...
from coalib.misc.CachingUtilities import (
    ...
    delete_cache_files, config_file_changed, update_config_file_db)
...
class FileCache:
    ...
    def __init__(self, log_printer, project_dir, flush_cache=False):
        ...
        if not time_consistent(log_printer, self.md5sum):
            ...
            flush_cache = True
        if config_file_changed(log_printer, self.md5sum, self.project_dir):
    ...
    def write(self):
        ...
        update_config_file_db(self.log_printer, self.md5sum, self.project_dir)
        self.changed_files = set()
